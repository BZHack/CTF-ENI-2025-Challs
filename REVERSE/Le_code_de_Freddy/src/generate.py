...
def generate(argv):
  ...
  userdef = ''.join(random.choice(userdef_charset) for _ in range(6))

  template = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'le_code_de_freddy.jinja'), 'r').read()
